...
def lines_detect(image):
    ...
    color_height = np.mean(
        rgb2gray(crop(image, (597, 0, 619, 720), copy=False)), axis=1
    )
...
class RewardCommission(UI, InfoHandler):
    ...
    def _commission_choose(self, daily, urgent):
        ...
        if no_shortest.count + running_count < self.max_commission:
            if daily.count:
                ...
                shortest = COMMISSION_FILTER.apply(
                    daily[::-1], func=self._commission_check
                )
    ...
    def _commission_start_click(
        self, comm, is_urgent=False, skip_first_screenshot=True
    ):
        """
        Start a commission.
        ...
        comm_timer = Timer(7)
        # Use configured timeout for ship check, or default 15 seconds
        ship_check_timeout = (
            self.config.Commission_ShipCheckTimeout
            if self.config.Commission_EnableShipCheck
            else 0
        )
        no_ship_timer = (
            Timer(ship_check_timeout).start()
            if self.config.Commission_EnableShipCheck
            else None
        )
        count = 0
        while 1:
            ...
            if self.appear(COMMISSION_ADVICE, offset=(5, 20), interval=7):
                ...
                self.interval_clear(COMMISSION_START)
                comm_timer.reset()
                # Only reset timer on first click, let it run for subsequent clicks
                if count == 1 and no_ship_timer is not None:
                    no_ship_timer.reset()
                continue
            # Check for insufficient ships
            # After clicking COMMISSION_ADVICE, if COMMISSION_START doesn't appear within timeout,
            # it's likely due to insufficient ships meeting commission requirements
            if count > 0 and no_ship_timer is not None and no_ship_timer.reached():
                logger.warning(
                    "Commission start timed out after clicking recommend button"
                )
                logger.warning(
                    "Likely cause: No ships meet commission requirements, skip this commission"
                )
                # Set custom status message to notify user
                try:
                    import os
                    ...
                    logger.warning(f"Failed to set custom status message: {e}")
                return False
            # Prevent stuck timer from triggering while waiting for ships
            # Clear stuck record periodically when we're actively waiting for COMMISSION_START
            if (
                count > 0
                and no_ship_timer is not None
                and no_ship_timer.current_time() > 5
            ):
                self.device.stuck_record_clear()
            # Check for commission list flashing bug
            # Only trigger this after we've given enough time for ships to appear
            if count >= 3 and (
                no_ship_timer is None or no_ship_timer.current_time() > 10
            ):
        ...
        with self.stat.new(
            ...
            while 1:
                ...
                if click_timer.reached() and self.appear_then_click(
                    REWARD_1, offset=(20, 20), interval=1
                ):
                    ...
                    continue
                if click_timer.reached() and self.appear_then_click(
                    REWARD_1_WHITE, offset=(20, 20), interval=1
                ):
                    self.interval_reset(GET_SHIP)
                    click_timer.reset()
                    reward = True
                ...
                if click_timer.reached() and self.appear_then_click(
                    ...
                    continue
                if click_timer.reached() and self.appear_then_click(
                    REWARD_GOTO_COMMISSION_WHITE, offset=(20, 20)
                ):
                    self.interval_reset(GET_SHIP)
                    click_timer.reset()
                    continue
    ...
    def run(self):
        ...
        if self.config.cross_get(
            keys="GemsFarming.GemsFarming.CommissionLimit", default=False
        ):
            daily = self.daily.select(category_str="daily", status="pending").count
            filtered_urgent = self.comm_choose.intersect_by_eq(
                self.urgent.select(status="pending")
            ).count
            logger.info(
                f"Daily commission: {daily}, filtered_urgent: {filtered_urgent}"
            )
            if daily > 0 and filtered_urgent >= 1:
                logger.info("Having daily commissions to do, delay task `GemsFarming`")
                self.config.task_delay(
                    minute=120,
                    target=future_finish if len(future_finish) else None,
                    task="GemsFarming",
                )
            elif filtered_urgent >= 4:
                logger.info(
                    "Having too many urgent commissions, delay task `GemsFarming`"
                )
                self.config.task_delay(
                    minute=120,
                    target=future_finish if len(future_finish) else None,
