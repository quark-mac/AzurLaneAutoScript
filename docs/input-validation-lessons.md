...
## 坑 8：清空输入框后立即弹回默认值，打断用户输入

### 问题现象

在 WebUI 中清空一个配置项（如关卡名称）准备输入新值时，输入框在约 1 秒后被自动回填为默认值，导致用户打字被打断。

### 根本原因

`_save_config` 方法在处理空值时做了两件事：

```python
if not len(v_str):
    default = deep_get(self.ALAS_ARGS, k + ".value")
    modified[k] = default
    deep_set(config, k, default)       # 1. 将默认值写入配置文件（合理）
    valid.append(k)
    pin["_".join(k.split("."))] = default  # 2. 将默认值推回 UI 组件（打断用户）
```

第 2 行的 `pin[...] = default` 是 PyWebIO 的 pin widget 赋值操作，会立即更新浏览器中的输入框内容。这与第 1 行的后端保存同时发生，但 UI 更新是即时的，在用户看来就是"刚清空就弹回来了"。

### 解决方案

删除 `pin["_".join(k.split("."))] = default` 行。默认值仍然写入配置文件，但不再推回 UI 组件。效果：

| 操作 | 结果 |
|------|------|
| 清空字段 → 输入新值 | UI 无弹回，正常保存新值 |
| 清空字段 → 不继续操作 | 配置文件已写默认值，刷新页面后显示默认值 |

### 经验

**PyWebIO 的 `pin[key] = value` 会实时推送到浏览器 DOM，而配置文件写入是后台操作——两者不在同一个"用户感知时刻"。** 空值的默认值回填应该只发生在磁盘上，UI 恢复默认值应等用户主动刷新页面或手动触发。
