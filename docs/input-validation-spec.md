...
当用户在 WebUI 中清空一个输入框时：

1. 空值**不触发表单验证**（不会出现红框和错误提示）
2. 系统**立即将默认值写入配置文件**（后端配置已更新）
3. 但**UI 不会自动回填默认值**（用户看到空白字段，可以不受干扰地输入新值）
4. 如果用户输入了新值 → 正常保存，覆盖默认值
5. 如果用户离开页面后刷新 → 从配置文件加载默认值显示

这样设计的原因：用户清空字段通常是为了输入新值，如果立即弹回默认值会打断输入流程。
